 ...
 *
 * @param request An object containing key-value pairs to be converted into a query string
 * @returns HttpParams to be used with HttpClient
 * @author thoaidc
 */
export const createSearchRequestParams = (request?: any): HttpParams => {
  let options: HttpParams = new HttpParams();

  if (request) {
    Object.keys(request).forEach(key => {
      if (key !== 'sort' && (request[key] || request[key] === 0 || request[key] === false)) {
        for (const value of [].concat(request[key]).filter(v => v !== '')) {
    ...
    if (request.sort) {
      request.sort.forEach((val: string) => {
